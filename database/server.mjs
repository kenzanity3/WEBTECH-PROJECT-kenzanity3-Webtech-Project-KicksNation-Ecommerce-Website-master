...
app.put('/api/admins/:Adminid', async (req, res) => {  // Fixed route parameter
    ...
    const adminIndex = admins.findIndex(admin => admin.Adminid == parseInt(adminId)); // Find the admin by ID
    if (adminIndex === -1) {
        return res.status(404).json({ error: 'Admin not found' }); // If admin not found, send a 404 response
    }

    admins[adminIndex] = {
        ...admins[adminIndex], // Keep existing admin data
...
app.put('/api/users/:Userid', async (req, res) => {
    ...
    const userIndex = users.findIndex(user => user.Userid === parseInt(userId)); // Find the user by ID
    if (userIndex === -1) {
    ...
    res.status(200).json({ message: 'User information updated successfully!', user: users[userIndex] });
});
...
app.delete('/api/users/cart/:Userid', async (req, res) => {
    const userId = parseInt(req.params.Userid); // Get the UserId from the request parameters
    ...
    if (Accesscontrol === 1) {
        ...
        orderformat.Subtotal = subtotal;
        orderformat.Tax = orderformat.Subtotal * taxRate;
        orderformat.Total = orderformat.Subtotal + orderformat.Tax;
...
app.post('/api/items', async (req, res) => {
    const itemInfo = req.body; // Extract item data from the request body
    ...
    itemInfo.Itemid = newItemId; // Assign the new Item ID to itemInfo

    items.push(itemInfo); // Add the new item data to the array

    await writeItems(items); // Write the updated item data back to the file

    res.status(201).json({ message: 'Item registered successfully', data: itemInfo });
});
...
app.delete('/api/items/:Itemid', async (req,res) =>{
    ...
    const itemIndex = items.findIndex(item => item.Itemid === parseInt(itemId)); // Find the item by ID
    if (itemIndex === -1) {
