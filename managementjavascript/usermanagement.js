...
export async function Userchangedetail(changedata) {
    const id = userID(); // Call the userID function to get the current user's ID
...
export async function deletecartitems(CartItems) {
    ...
    try {
        ...
        alert('Cart item(s) deleted successfully!'); // Confirmation message after successful deletion
        return true;
    } catch (error) {
        // Log the error and alert the user
