...
const usernamePattern = /^(?!\d)[A-Za-z][A-Za-z0-9]*$/;
...
document.addEventListener('DOMContentLoaded', (event) => {
    event.preventDefault(); // Prevent the default DOMContentLoaded behavior
    event.stopPropagation(); // Stop the event from propagating further

    window.Logout = Logout; // Assign Logout to the global scope
});
